...
def main():
    form = Tk()
    form.wm_title("Rock Band Emotion Maker")
    form.minsize(height=148, width=74)

    Label(form, text="Use velocity for the viseme parameters.").grid(row=0, column=0, pady=5, columnspan=2)
    Label(form, text="Use channel as a multiplier for velocity to reach 255.").grid(row=1, column=0, pady=5, columnspan=2)

    ttk.Separator(form, orient="horizontal").grid(row=2, column=0, columnspan=2)

    btn_fromnotes = Button(form, text="Notes to Text",
            width=22, justify = 'center', command=a3_notes_to_text)
    btn_fromnotes.grid(row=3, column=0, padx=3, pady=3, sticky="w")

    #btn_fromtext = Button(form, text="Text to Notes",
    btn_fromtext = Button(form, text="Coming Soon",
            width=22, state = 'disabled' 
            #command=a3_text_to_notes
            )
